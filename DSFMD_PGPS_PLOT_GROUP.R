...
# Example for using leaflet for interactive display of GPS tracks by Group
...
for (csdir in SubDirNames)
{
  ...
  IDV <-unlist(strsplit(FileNames,'.csv'))
  IDV <-as.data.table(t(as.data.table(strsplit(IDV,'_'))))
  if (ncol(IDV)<3)
  {
      IDV$EXP <- 'C'
  }
  colnames(IDV)<-c('DATE','ID','EXP')
  EXV <- unique(IDV$EXP) 
  ...
  factpal <- colorFactor(topo.colors(length(EXV)), EXV, na.color = "#808080")
  ...
  for (cfile in FileNames)
  {
    ...
    ll<-addPolylines(ll,data=sdf@coords, group=IDV$EXP[fdx] ,color=factpal(IDV$EXP[fdx]), weight = 2, opacity = 1)
  ...
  ll<-addCircleMarkers(ll,data=loc_dt[sdx,],
    ...
    addLayersControl(baseGroups = c("Positron", "Toner", "Satellite"), overlayGroups = c("Release Site","Home Loft","Vanishing Bearing", EXV),
                       options = layersControlOptions(collapsed = FALSE))
  
  #save plot
  dir.create(paste(mdir,"/plot/",sep=''))
  f<-paste(mdir,'/plot/',csdir,"_group.html",sep='')
